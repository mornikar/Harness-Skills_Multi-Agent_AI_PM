...
所有 Agent 间通信必须通过宿主平台的通信工具，遵循以下信封格式：
...
向 orchestrator 发送消息:
  ...
  content: |
    ...
    操作: 修改 src/router/index.ts — 添加todo路由
...
向 orchestrator 发送消息:
  ...
  content: |
    ...
    操作: 修改 tsconfig.json — 关闭strict模式
...
广播消息:
  ...
  content: |
    ...
    原因: 用户决定终止项目
    行动: 所有Agent请立即保存进度到HEARTBEAT，准备接收关闭请求
```
...
4. 追加变更日志
5. 尝试解除阻塞后 → 向子Agent发送 task_update 消息
```
...
2. 审阅内容：
   ...
   - 预估轮次是否在budget内？
3. 决策：
   - 通过 → 向子Agent发送 plan_approved
   - 驳回 → 向子Agent发送 plan_rejected + 修改建议
...
| 子Agent 未回复 shutdown_request | 3分钟 | orchestrator 强制清理 |
...
| orchestrator 的指令未被接收 | 子Agent长时间无响应 | 重新发送指令 |
| broadcast 消息丢失 | 个别子Agent行为与其他不一致 | 定向补发消息 |
| 通信通道整体故障 | 消息持续发送失败 | 重建通信通道 → 从检查点恢复 |
...
| P1（优先处理） | task_handoff | 需要尽快创建新Agent接手 |
