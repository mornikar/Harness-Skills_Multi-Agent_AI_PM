...
2. 总结已完成的工作和未完成的步骤
3. 通知 orchestrator：需要重启会话
4. orchestrator 重新创建 Agent 时，子 Agent 通过 HEARTBEAT 恢复
```
...
**恢复步骤**：
```
1. 修改文件更新自己 HEARTBEAT 状态为 ⚠️ blocked
2. 通知 orchestrator：
...
1. 分析错误类型：
   ...
   - 参数错误 → 修正参数后重试
   - 权限错误 → 通知 orchestrator
2. 最多重试 1 次
3. 仍然失败 → 通知 orchestrator
```
...
2. 分类：
   - 自身代码问题 → 修复代码后重新编译/测试
   - 依赖变化（上游 Agent 修改了接口）→ 通知 orchestrator
3. 最多重试 2 次
