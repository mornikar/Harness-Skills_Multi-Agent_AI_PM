...
**v3.1 Ironforge** — 三层架构多Agent协作开发框架

内核 + 10个独立Skill + 编排模板 · 安全/稳定/可观测企业工程化 · 平台无关
...
[![Agents](https://img.shields.io/badge/Skills-10+%E2%80%A2Engine-1A8CFF?label=Skills)](./pm-core/)
...
| 层 | 机制 |
|---|------|
| 平台物理层 | spawn mode + blocked_tools |
| Harness逻辑层 | 黄灯操作通知 + 拦截窗口 |
| SKILL.md行为层 | 编码规范·禁止事项 |
| 敏感信息防护 | 写入前正则扫描 |
| 审计日志 | JSONL + trace_id |
...
<details>
<summary><b>上下文自适应（v3 新增）</b></summary>
...
<details>
<summary><b>平台抽象层（v3 新增）</b></summary>
...
```
【旧】Phase 0 → Phase 1 → Phase 2（阻塞）→ Phase 3

【新】Phase 0 → Phase 1 → Phase 3（同时 Phase 2 并行进行）
                            ↓
              Phase 2 原型完成后 → 成为 Phase 3 研发的方向指标
```

**关键约束**：Phase 3 不等 Phase 2 完成；原型定型后切换为对齐原型开发。
...
<details>
<summary><b>Harness = 高解耦方向盘（v3.1 Ironforge）</b></summary>

每个 Agent 的 Harness 采用 **inherits 模式**：
- 公共层 `base/`（7个文件）：权限/安全/审计/检查点/交接/上下文/可观测性
- 特化层：每个 Harness 只保留权限覆盖、审计事件覆盖、稳定性覆盖

> 改一处生效全局。详见 [`harnesses/README.md`](./harnesses/README.md)
...
1. 创建 `pm-{agent-name}/SKILL.md`（行为规范）
2. 创建 `pm-{agent-name}/standalone-prompt.md`（独立运行 prompt）
3. 在 `harnesses/` 下创建 `{agent-name}-harness.md`（平台适配）
...
| 版本 | 日期 | 变更摘要 |
|------|------|---------|
| **v3.1** | 2026-04-24 | Ironforge 企业工程化：安全+稳定+可观测 + base/公共层 + web-design-engineer + Harness inherits |
